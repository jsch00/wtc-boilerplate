...
write_collection_env() { # <collection-dir> <collection-name>
  ...
  # Collection-scoped secrets. Created empty and never rewritten: .env.collection
  # above is regenerated wholesale on every branch-off, so anything hand-added
  # there is lost. This second file is the one place a secret scoped to THIS
  # collection survives, which the shared control root deliberately cannot be
  # (instructions/secrets.md: one canonical copy per file keeps rotation sane).
  # Listed in mise.toml unconditionally, so it must exist even when empty.
  if [ ! -f "$dir/.env.collection.local" ]; then
    cat > "$dir/.env.collection.local" <<'EOF'
# Collection-scoped secrets and overrides. NOT generated, NOT committed
# (the collection root is not a git repo), and never rewritten by harness tools.
#
